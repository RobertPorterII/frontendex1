...
export default function ProjectDetails({projects,setProjects}) {
    ...
    useEffect(() => {
        const fetchProject = async() => {
            const res = await fetch(`http://localhost:3500/api/projects/${params.id}`);
            const data = await res.json();
            console.log(data);

            setProject(data.project); // current project
            setUpdateFormData(data.project); // form data for updating project
    ...
    if (!project) {
        ...
        const handleSubmit = async (e) => {
            try {
                ...
                // const projectsCopy = [...projects]; no longer needed
